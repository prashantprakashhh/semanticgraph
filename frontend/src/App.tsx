...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState('ingest'); // ingest, explore
    ...
    <div className="flex h-screen bg-gray-900 text-white">
      <nav className="w-64 bg-gray-800 p-4 flex flex-col justify-between">
        <div>
          ...
          <ul>
            <li className={`mb-4 p-2 rounded-md cursor-pointer flex items-center ${currentPage === 'ingest' ? 'bg-blue-600' : 'hover:bg-gray-700'}`}
                onClick={() => setCurrentPage('ingest')}>
                <Upload className="mr-3 h-5 w-5" /> Ingestion
            </li>
            <li className={`p-2 rounded-md cursor-pointer flex items-center ${currentPage === 'explore' ? 'bg-blue-600' : 'hover:bg-gray-700'}`}
